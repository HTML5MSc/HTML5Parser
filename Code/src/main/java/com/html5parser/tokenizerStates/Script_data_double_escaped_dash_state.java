...
public class Script_data_double_escaped_dash_state implements ITokenizerState {
	...
	public ParserContext process(ParserContext context) {
		...
		if(context.isTracing())
			context.getTracer().addParseEvent("8.2.4.30", currentChar);
		...
		case NULL:
			/*
			 * Parse error. Switch to the script data escaped state. Switch to
			 * the script data double escaped state Emit a U+FFFD REPLACEMENT
